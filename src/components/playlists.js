  ...
  //Update la liste de morceaux quand un morceau est ajouté/supprimé
    useEffect(() => {
      if(localStorage.getItem("playlistName") != null){
        playlistTracks(localStorage.getItem("playlistName"), localStorage.getItem("playlistID"), localStorage.getItem("userID"));
      }
    }, [refreshTracks]);
  ...
  return (
    <div>
      ...
      {tracks !== null ? (
        <Tracks
          data={tracks}
          playlist={localStorage.getItem("playlistName")}
          update={updateTracks}
        />
      ) : (
        <div> </div>
